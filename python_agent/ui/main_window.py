...
class BeavisMainWindow(QMainWindow):
    ...
    def _build_apps_catalog_rows(self) -> list[dict[str, object]]:
        ...
        for change in self._pending_app_changes.values():
            ...
            if operation == "enable":
                ...
                row["enabled"] = True
                row["status"] = "Черновик · будет включено"
            ...
            if operation == "disable":
                row["enabled"] = False
                row["status"] = "Черновик · будет отключено"
                row["pending_change"] = change
                row.pop("record", None)
                row["editable"] = True
                rows_by_id[app_id] = row
    ...
    def _effective_app_ids_after_pending(self) -> set[str]:
        ...
        for change in self._pending_app_changes.values():
            ...
            elif operation in {"add", "enable", "update_speech_forms"}:
                app_ids.add(app_id)
        return app_ids

    def _find_catalog_app_by_id(self, app_id: str) -> dict[str, object] | None:
        app_id = app_id.strip()
        if not app_id:
            return None

        for row in self._build_base_apps_catalog_rows():
            if str(row.get("app_id", "")).strip() == app_id:
                return row

        return None

    def _pending_change_for_app(self, app_id: str) -> dict[str, object] | None:
        app_id = app_id.strip()
        if not app_id:
            return None
        return self._pending_app_changes.get(app_id) or self._pending_app_changes.get(f"add:{app_id}")
    ...
    def _validate_app_form(self) -> None:
        ...
        pending = self._pending_change_for_app(app_id)
        if pending and str(pending.get("operation", "")) != "delete":
            self.app_id_status_label.setText(f"app_id уже есть в черновике: {app_id}")
            self.app_id_status_label.setProperty("state", "error")
            self.add_app_button.setEnabled(False)
            self._refresh_widget_style(self.app_id_status_label)
            return

        existing = self._find_catalog_app_by_id(app_id)
        if existing is not None:
            source = str(existing.get("source", "user") or "user")
            enabled = bool(existing.get("enabled", True))
            if enabled:
                self.app_id_status_label.setText(
                    f"app_id уже есть в каталоге ({source}); будут обновлены фразы запуска"
                )
            else:
                self.app_id_status_label.setText(
                    f"app_id есть в каталоге ({source}), но отключён; будет включён и обновлён"
                )
            self.app_id_status_label.setProperty("state", "ok")
            self.add_app_button.setEnabled(True)
        else:
    ...
    def _submit_user_app(self) -> None:
        ...
        if not app_id:
            self._validate_app_form()
            self._set_status("app_id пустой", "error")
            return

        pending = self._pending_change_for_app(app_id)
        if pending and str(pending.get("operation", "")) != "delete":
            self._validate_app_form()
            self._set_status("app_id уже есть в черновике", "error")
            return
        ...
        if existing is not None:
            change: dict[str, object] = {
                ...
                "display_name": display_name or str(existing.get("display_name", "")),
                "app_id": app_id,
                "speech_forms": speech_forms,
            }
        else:
...
# --- BEAVIS_HOME_PAGE_FULL_PATCH_V1 ---
# Встроенный редизайн только главной страницы Beavis.
# Остальные страницы остаются на старых методах BeavisMainWindow.
...
def _beavis_install_home_page_embedded(window_cls) -> None:
    ...
    window_cls._beavis_home_full_patch_installed = True

try:
    _beavis_install_home_page_embedded(BeavisMainWindow)
except Exception as _beavis_home_error:
    print(f"[Beavis UI] embedded home page patch was not applied: {_beavis_home_error}")
# --- /BEAVIS_HOME_PAGE_FULL_PATCH_V1 ---
